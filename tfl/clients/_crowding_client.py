"""Client to interact with the Crowding API."""
...
from tfl.enums import DayOfWeekEnum

__all__ = ["CrowdingClient"]


class CrowdingClient(TFLClient):
    """Client to interact with the Crowding API.
    ...
    Example:
        ...
        async with CrowdingClient(auth=Auth(key="<your-tfl-api-key>")) as client:
            response = await client.get_crowding(naptan_code="940GZZLUBND", day=DayOfWeekEnum.MONDAY)
    ...
    async def get_crowding(self, naptan_code: str, day: Optional[DayOfWeekEnum] = None) -> httpx.Response:
        ...
        if day:
            _url += f"/{day.value}"
        return await self.get(url=_url)
